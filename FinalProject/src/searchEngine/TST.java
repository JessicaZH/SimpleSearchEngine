...
public class TST<Value> {
    ...
    **************************************************************/
    ...
    /*********************************************************************************************
     * Insert string s into the symbol table and the val is the of occurrences in the file.
     *********************************************************************************************/
     public void put1(String s, Integer val) {
         if (!contains(s)) N++;
         root = put1(root, s, val, 0);
     }
     
     
     private Node put1(Node x, String s, Integer val, int d) {
    	 char c = s.charAt(d);
         if (x == null) {
             x = new Node();
             x.c = c;
         }
         if      (c < x.c)             x.left  = put1(x.left,  s, val, d);
         else if (c > x.c)             x.right = put1(x.right, s, val, d);
         else if (d < s.length() - 1)  x.mid   = put1(x.mid,   s, val, d+1);
         else                          
        	 {
        	 	if (x.val == null)
        	 		x.val = val+1;	
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        while (str.hasMoreTokens()) {
        	//st.put(str.nextToken(), i);
        	st.put1(str.nextToken(), i);
        	//i++;
        }
        // print results
        /*for (String key : st.keys()) {
