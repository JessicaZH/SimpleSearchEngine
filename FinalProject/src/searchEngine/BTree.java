...
public class BTree<Key extends Comparable<Key>, Value>  {
    ...
    public ArrayList<Value> get(Key key) { return search(root, key, HT); }
    ...
    private ArrayList<Value> search(Node x, Key key, int ht) {
        ...
        if (ht == 0) {
            for (int j = 0; j < x.m; j++) {
                if (eq(key, children[j].key)) return (ArrayList<Value>) children[j].value;
            }
        }
    ...
    private Node insertVal(Node h, Key key, Value value, int ht) {
        ...
        if (ht == 0) {
            for (int j = 0; j < h.m; j++) {
                if (eq(key, children[j].key)) children[j].value.add(value);
            }
        }
        ...
        // internal node
            ...
            for (int j = 0; j < h.m; j++) {
                if (j+1 == h.m || less(key, children[j+1].key))
                    return insertVal(children[j].next, key, value, ht-1);
            }
        }
        return null;   
	}
    ...
    public static void writeFile(String string) {
   	...
   	try {
			String fileName = curPath +"//memoryManagement//B-tree.dat";
    ...
    *************************************************************************/
    public static void main(String[] args) {
        ...
        In in = new In(".//ChIP-seq-reads//ChIP-seq-reads-1M.dat");
        String str;
        Integer i=1;
        ...
        while (!in.isEmpty()) {
        	str=in.readLine();
        	st.put(i.toString(),str);
        	i++;
        }
        endtime=System.currentTimeMillis();
        StdOut.println("CPU is "+ (endtime-starttime));
        //writeFile(st.toString());
